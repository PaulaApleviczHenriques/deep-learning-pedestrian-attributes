...
Original file is located at
    https://colab.research.google.com/drive/11vwCyg6wpMITcOjkXVCfk8_cPb9laR6B
...
def load_all_labels(txt_path):
    ...
    with open(txt_path, "r") as f:
        ...
        for row in reader:
            ...
            top_color = int(row[TOP_COL_IDX].strip()) - 1
            bottom_color = int(row[BOTTOM_COL_IDX].strip()) - 1
            gender = int(row[GENDER_COL_IDX].strip())
            bag = int(row[BAG_COL_IDX].strip())
            hat = int(row[HAT_COL_IDX].strip())
...
class MultiTaskModel(nn.Module):
    def __init__(self):
        ...
        # classificação uma para cada atributo
