...
Original file is located at
    https://colab.research.google.com/drive/1N6TErd2derOxigp_92BadH-M-ugtVbrb
...
SAVE_PATH = '/content/drive/MyDrive/Ciência da Computação/Experiência Criativa: Projeto Transformador II/best_mobilenet_multitask.pth'
...
# Modelo Multi-Task com MobileNet V2
class MobileNetMultiTask(nn.Module):
    def __init__(self):
        super(MobileNetMultiTask, self).__init__()

        # Backbone compartilhado (MobileNet V2)
        mobilenet = models.mobilenet_v2(weights='IMAGENET1K_V1')
        # Usar só as features (remove classifier)
        self.backbone = nn.Sequential(*list(mobilenet.children())[:-1])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

        # MobileNet V2 tem 1280 features
        self.fc_top_color = nn.Linear(1280, 11)     # 11 cores
        self.fc_bottom_color = nn.Linear(1280, 11)  # 11 cores
        self.fc_gender = nn.Linear(1280, 2)         # 2 classes
        self.fc_bag = nn.Linear(1280, 2)            # 2 classes
        self.fc_hat = nn.Linear(1280, 2)            # 2 classes

    def forward(self, x):
        # Features compartilhadas
        features = self.backbone(x)
        features = self.avgpool(features)  # Global average pooling
        features = features.view(features.size(0), -1)  # Flatten para (batch, 1280)
...
model = MobileNetMultiTask().to(DEVICE)
...
print("\nTreinando MobileNet V2 Multi-Task...")
...
for epoch in range(1, EPOCHS+1):
    ...
    # Salvar melhor modelo no Google Drive
    if te_avg > best_avg_acc:
        best_avg_acc = te_avg
        torch.save(model.state_dict(), SAVE_PATH)
        print(f"Melhor modelo salvo no Drive! Avg Acc: {best_avg_acc*100:.2f}%")
